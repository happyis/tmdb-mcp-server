...
`;
    ...
    server.tool(
      ...
      async ({ movieId }) => {
        ...
        const crew = credits.crew?.filter((c: any) => ['Director', 'Producer', 'Writer'].includes(c.job))
    ...
    server.tool(
      ...
      async ({ query, page }) => {
        ...
        const resultText = response.results.map((person: any, idx: number) => {
          return `${idx + 1}. ${person.name}
   알려진 작품: ${person.known_for_department || '정보 없음'}
   대표작: ${person.known_for?.map((k: any) => k.title || k.name).join(', ') || '정보 없음'}
   TMDB ID: ${person.id}
`;
        }).join('\n');
    ...
    server.tool(
      ...
      async ({ movieId }) => {
        ...
        const posters = images.posters?.slice(0, 5).map((poster: any, idx: number) => 
          `${idx + 1}. 포스터 (${poster.width}x${poster.height}) - https://image.tmdb.org/t/p/w500${poster.file_path}`
        ).join('\n') || '포스터 정보 없음';

        const backdrops = images.backdrops?.slice(0, 5).map((backdrop: any, idx: number) => 
          `${idx + 1}. 백드롭 (${backdrop.width}x${backdrop.height}) - https://image.tmdb.org/t/p/w1280${backdrop.file_path}`
        ).join('\n') || '백드롭 정보 없음';
    ...
    server.tool(
      ...
      async ({ movieId }) => {
        ...
        const videoList = videos.results.slice(0, 10).map((video: any, idx: number) => {
          const youtubeUrl = video.site === 'YouTube' ? `https://www.youtube.com/watch?v=${video.key}` : '링크 없음';
          return `${idx + 1}. ${video.name} (${video.type})
   사이트: ${video.site}
   링크: ${youtubeUrl}`;
        }).join('\n\n');
    ...
    server.tool(
      ...
      async ({ movieId, page }) => {
        ...
        const movies = response.results.slice(0, 10).map((movie: Movie, idx: number) => 
          `${idx + 1}. ${movie.title} (${movie.release_date?.split('-')[0] || '연도 미상'})
   평점: ${movie.vote_average}/10
   개요: ${movie.overview?.substring(0, 100) || '개요 없음'}...`
        ).join('\n\n');
...
- 알려진 분야: ${person.known_for_department || '정보 없음'}
- 인기도: ${person.popularity || 0}/100

## 프로필
${person.biography || '프로필 정보가 없습니다.'}

TMDB 링크: https://www.themoviedb.org/person/${personId}
`;
   ...
   평점: ${movie.vote_average || 0}/10`
        ).join('\n') || '출연작 정보 없음';

        const crew = credits.crew?.slice(0, 5).map((movie: any, idx: number) => 
   ...
   평점: ${movie.vote_average || 0}/10`
    ...
    server.tool(
      ...
      async ({ query, page }) => {
        ...
        const resultText = response.results.map((tv: any, idx: number) => {
...
- 에피소드 수: ${tv.number_of_episodes}개
- 시즌 수: ${tv.number_of_seasons}개
- 장르: ${tv.genres?.map((g: Genre) => g.name).join(', ') || '정보 없음'}
- 평점: ${tv.vote_average}/10 (${tv.vote_count}명 평가)
...
## 제작사
${tv.production_companies?.map((c: ProductionCompany) => c.name).join(', ') || '정보 없음'}

TMDB 링크: https://www.themoviedb.org/tv/${tvId}
`;
    ...
    server.prompt(
      ...
      "영화 추천 프롬프트",
      () => {
        const promptText = `저는 영화를 좋아하는 사용자입니다. 저의 취향과 상황에 맞는 영화를 추천해주세요. 추천할 때는 제목, 감독, 주요 배우, 장르, 간단한 줄거리를 포함해 주세요.`;
